...
def _build_history() -> list[MessageSchema]:
    return [
        MessageSchema(
            user_id="user-1",
            user_nickname="阿虚",
...
def test_rewrite_query_returns_original_without_history(monkeypatch: Any) -> None:
...
def test_rewrite_query_retries_then_succeeds(monkeypatch: Any) -> None:
    ...
    service = query_rewrite_module.QueryRewriteService()
    result = asyncio.run(service.rewrite_query("她是谁", _build_history()))
