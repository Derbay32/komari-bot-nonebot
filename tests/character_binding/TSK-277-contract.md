...
- `finish_send(reply)` 由 handler 在发送尝试的 `finally` 中调用：确认成功的
  `handle_event` 只登记 completed 回复、**不**清理临时会话；发送尝试结束后
  （成功、发送失败、发送前重审拒绝）才恰好调用一次 `coordinator.cancel`。
  `finish_send` 幂等，对非 completed 回复是 no-op；`close()` 会清理尚未
  finish 的 completed 回复。
...
- 只处理精确 `GroupAtMessageCreateEvent` 且 `content` 以 `/bind` 开头的入站；
  命令边界按词法判定：门禁按 strip 后的精确 `/bind` 消耗初始 claim，handler/
  wizard 必须同样接受 `" /bind "` 并发出挑战（不得消耗 claim 而不发挑战）；
  `/bindfoo` 不是命令、必须静默，而 `/bind <未知子命令>` 仍返回
  `绑定命令格式不正确，请发送 /bind 查看当前步骤。`；
...
- 发送尝试包在 `try/finally` 中，`finally` 无条件调用 `wizard.finish_send(reply)`：
  成功、发送失败与发送前重审拒绝三种路径都清理一次临时会话；
...
- **成功发送与临时会话清理的次序**：确认成功的 `handle_event` 不得清理临时会话
  （`coordinator.cancel` 不在其中），否则成功回复的发送前重审会因 token 失效而
  失败。handler 在发送尝试的 `finally` 调用 `finish_send(reply)`，成功、发送失败
  与发送前重审拒绝三种路径都恰好清理一次；`finish_send` 幂等且对非 completed
  回复 no-op。274 的 cancel 语义保持即时撤销（不做“保留权限”的变体）。
- **completed 回复不得绕过重审**：`authorize_send` 先执行 `coordinator.recheck`；
  重审拒绝时必须返回 False，即使 canonical 已等于目标（新封禁/群撤销在回复
  生成后生效也不得放行）。canonical 回退只允许在重审通过之后作为发送授权的
  补充，不得替代重审。重审必须是 `authorize_send` 的最后一步：任何 canonical
  读取（await）都必须发生在重审之前，读取期间的封禁/策略变更不得漏检。
- **组锁等待后必须重审**：confirm 在获得 `lock_group_scope` 之后、写入之前
  必须再次复核准入（以及会话 TTL/generation）；等待锁期间发生策略撤销或 TTL
  到期时零写入、静默返回。
- **持锁期间不得跨 session 再取同组锁**：confirm 持有组锁时调用的任何 resolver
  （包括 `init_plugin` 装配的生产 group/member resolver，它们各自 `_open_session`
  后经 `BindingTransaction.resolve_*` 申请同一 advisory lock）都会形成跨 session
  自锁。真实 `init_plugin` 安装的生产 resolver 下，正式 handler 全链必须在有界
  时间内完成并发出成功文案；测试以 `lock_timeout`/`statement_timeout` 与终止
  advisory 等待后端兜底，不允许无限挂起。
- **最终 commit 窗口必须闭合**：`_apply` 完成 bind/flush 之后、`commit()` 之前
  仍存在多个 await 窗口；此时 cancel/close、策略/封禁撤销或 TTL 到期必须使
  暂存事务回滚、零正式行且不返回成功回复。写入与最终 commit 之间不得存在
  未受保护的失效窗口。
- **同 scope 串行或不可变快照**：同一 `(app_id, group_openid, member_openid)`
  的 `handle_event` 必须串行化，或在进入 await 前冻结草稿快照并在写入前校验；
  confirm 等待期间发生的 cancel/改草稿不得使 confirm 写入未确认的新名字。

## AC → 测试映射

| AC | 测试节点 |
| --- | --- |
| 1 命令/按钮/纯文本可达/无 fallback | `test_tsk277_handler.py::test_real_handler_sends_markdown_challenge_with_quote_and_no_second_claim`、`test_tsk277_handler.py::test_real_handler_ignores_non_bind_and_non_native_events`、`test_tsk277_handler.py::test_real_handler_send_failure_does_not_append_fallback`、`test_tsk277_handler.py::test_real_handler_accepts_trimmed_initial_bind_without_losing_claim`、`test_tsk277_handler.py::test_real_handler_ignores_bind_prefixed_non_command_but_reports_unknown_subcommand`、`test_tsk277_wizard.py::test_first_unmapped_bind_sends_one_challenge_and_repeat_is_silent`、`test_tsk277_wizard.py::test_legacy_candidate_only_after_verified_active_bind`、`test_commands.py::test_character_binding_has_no_onebot_bind_command_matchers` |
...
| 8 原子事务/并发/不确定提交/覆写保护/缓存 | `test_tsk277_pg.py::test_confirm_commits_atomically_and_publishes_cache_only_after_commit`、`test_tsk277_pg.py::test_group_and_member_conflicts_preserve_original_records`、`test_tsk277_pg.py::test_same_group_concurrent_name_has_one_winner_and_loser_can_rename`、`test_tsk277_pg.py::test_commit_outcome_unknown_after_commit_does_not_report_and_converges`、`test_tsk277_pg.py::test_commit_outcome_unknown_before_commit_leaves_no_record_and_repeat_writes_once`、`test_tsk277_pg.py::test_commit_unknown_after_commit_never_overwrites_later_maintenance_rename`、`test_tsk277_pg.py::test_commit_unknown_before_commit_never_overwrites_existing_new_formal_state`、`test_tsk277_pg.py::test_concurrent_name_change_during_confirm_never_writes_unconfirmed_name` |
| 9 效果前重审/state token/映射≠白名单 | `test_tsk277_handler.py::test_send_time_recheck_suppresses_reply_in_wait_window`、`test_tsk277_handler.py::test_positive_control_send_performs_send_time_recheck`、`test_tsk277_handler.py::test_restricted_group_stays_silent_even_for_configured_superuser`、`test_tsk277_handler.py::test_real_handler_sends_markdown_challenge_with_quote_and_no_second_claim`、`test_tsk277_wizard.py::test_denied_recheck_suppresses_visible_reply_and_draft_effect`、`test_tsk277_wizard.py::test_mapped_pending_evidence_advances_to_legacy_without_reclaim`、`test_tsk277_pg.py::test_real_handler_evidence_progression_and_success_send_survive_cancel`、`test_tsk277_pg.py::test_authorize_send_denies_completed_reply_after_recheck_revocation`、`test_tsk277_pg.py::test_confirm_rechecks_after_group_lock_and_writes_nothing_when_revoked`、`test_tsk277_pg.py::test_real_coordinator_denies_send_after_post_commit_ban_change`、`test_tsk277_pg.py::test_confirm_lock_wait_ttl_expiry_writes_nothing`、`test_tsk277_pg.py::test_real_handler_denied_send_finally_revokes_completed_session`、`test_tsk277_pg.py::test_apply_pause_then_invalidation_rolls_back_and_returns_no_reply`、`test_tsk277_pg.py::test_authorize_send_rechecks_after_canonical_read`、`test_tsk277_pg.py::test_real_init_plugin_resolvers_do_not_self_deadlock_on_confirm` |
| 10 wizard 拥有事务与幂等完成态 | `test_tsk277_pg.py::test_confirm_commits_atomically_and_publishes_cache_only_after_commit`、`test_tsk277_pg.py::test_real_handler_evidence_progression_and_success_send_survive_cancel`、`test_tsk277_pg.py::test_real_handler_send_failure_finally_revokes_completed_session`、`test_tsk277_pg.py::test_real_handler_denied_send_finally_revokes_completed_session`、`test_tsk277_wizard.py::test_finish_send_is_noop_for_non_completed_replies`、`test_tsk277_pg.py::test_cancel_close_reset_during_lock_wait_writes_nothing`、`test_tsk277_pg.py::test_init_plugin_installs_and_close_plugin_removes_wizard`、`test_tsk277_pg.py::test_apply_pause_then_invalidation_rolls_back_and_returns_no_reply` |
