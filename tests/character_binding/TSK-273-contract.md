...
- `get_runtime_collectors()` 和 `set_runtime_collectors(sequence)`，作为 OneBot
  listener 的唯一运行时注入口；空序列表示未配置，多个 app collector 必须同时保留；
...
流程方向固定为：OneBot 先缓存人类以原生 `@` 官方 QQ 发出的 `/bind` 命令；官方 Bot
再发送一条带会话码、原生引用该命令的挑战；OneBot 收到挑战后沿 reply 读取原始命令。
QQ `open_session` 可以在 OneBot 原消息之后到达。`event.message` 可以
...
是正常原生 reply；完整 `get_msg` 返回 `message_id` 与 native quote target 完全一致、
`message_type=group`、有完整 `group_id`、
同一群、被引用发送者是人类成员、同一正文；QQ 原始消息 ID 与会话一致；当前原始
消息缓存与完整 `get_msg` 一致。`get_msg.group_id` 缺失、读取失败、跨群、跨成员、
正文、消息 ID 或消息类型不符均故障关闭。普通聊天和缺少原生 @ 的 `/bind` 不得进入
可用原消息缓存。重复合法事件幂等，不重复取证或产生绑定。`get_msg` 异步挂起期间若
会话被取消、连接代次重置或到达 600 秒边界，释放后的旧任务不得产生证据。

## 监听与准入

实际导入 `komari_bot.plugins.character_binding`（不是测试中的 package shim）时，
`reply_evidence` 注册一个 `message` matcher，且 `block=False`。它在已获准的群
消息 matcher 阶段从 `get_runtime_collectors()` 取得实例并分发消费证据；受
`group_admission` 拦截的群事件不得进入监听器、不得写入原消息缓存。监听器没有可见
OneBot 发送副作用。
