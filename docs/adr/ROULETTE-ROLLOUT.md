...
- 参阅[群指令面板配置与验收](QQ-COMMAND-MENU.md)注册客户端菜单；不要把群指令面板与消息下方按钮或 C2C 自定义菜单混为一谈。
...
- 在有效会话内再次确认已完成的绑定、改名或解绑；检查返回“本次操作已完成，无需再次确认。”，不把这条回执当成再次修改。
...
完成全部自动化与 CI 验收后，固定已获授权的实际运行版本。
按下文的代表性样本口径选择人工检查项；以下场景表用于回归参考，不要求靠随机对局逐项制造全部状态：

| 样本 | 操作与检查 |
|---|---|
| 首次群绑定 | 由两名不同用户分别 @官 Bot 发起 `/bind`；检查挑战各自原生引用正确，OneBot 不额外发言，确认前无正式名字。 |
| 手动填充按钮 | 点击姓名、确认和游戏按钮；检查只填入输入框，未手动发送前不执行动作。 |
| 空道具提示 | 打开没有道具的面板；检查标题下方显示“没有任何道具，在自己回合进行额外开枪时即可抽取道具”。 |
| 道具获得提示 | 在自己回合额外开枪后查看新回复；检查成功入库的道具显示本次名称与数量，不把已有库存算作新获得。 |
| 道具入口 | 点击有道具时的新状态卡“道具”按钮；检查填入 `/轮盘 道具`，而非使用或丢弃命令。 |
| 面板操作 | 手动发送 `/轮盘 道具`；检查使用、丢弃按钮位于面板中，且遵守实际库存与合法目标限制。 |
| 部分入库奖励 | 查看连发后部分入库、部分待选的新回复；检查已获得道具与当前待选道具分别展示，不提前披露后续奖励类型。 |
...
| 最终终局 | 由非胜者开枪淘汰或弃权触发终局；检查只有一条完整单段正文，无引用、粗体、分割线、玩家名单或按钮，恰好提及胜者而非触发者。 |
| 本群排行榜 | 发送 `/轮盘 排行榜`；检查该次胜利只增加一次，展示本群榜单。 |

- 通过正常游戏产生选定的人工样本。
- 禁止改库、伪造库存或强制补发来制造人工样本。
...
- 区分实际运行版本的真人样本与自动化覆盖，不将旧原型样本自动转为本次通过。

### 本轮验收结论（2026-09-15，UTC+8）

- 采用用户确认的代表性真人提及样本，验证共用原生提及通道的客户端展示。
- 保留各场景的自动化目标选择检查，不以共用提及格式代替业务裁决验证。
- 确认双用户绑定、正常对局、道具反馈与入口、终局胜者提及、本群排行榜已获得真人通过反馈。
- 记录回合轮转提醒、满仓奖励选择提醒、成功上锁提醒未单独获得本轮真人确认。
- 复用已通过的自动化检查覆盖上述提醒分支，不要求为相同平台提及通道重复随机开局。
- 保留未单独实测的场景说明，不将它们写成真人通过，也不再因此阻塞本轮验收。
- 参阅[最终验收证据](ROULETTE-ACCEPTANCE.md)，核对版本、检查结果与未覆盖条件。
