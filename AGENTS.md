...
- TSK-196：图片理解失败统一汇总与安全终止。delegated 部分失败（非全部可用索引均已尝试且失败）保留结构化 `read_image` 失败工具结果并允许 `final_response`，成功任务把聚合摘要附加到 `ReplyResult.image_diagnostic`；全部可用索引均已尝试且失败（含同轮 `read_image`+`final_response`）由 `_execute_tool_loop` 以 `ImageUnderstandingFailureError`（只携带 `ImageFailureSummary`，`str()` 仅含模式）终止，最后一次失败工具 Trace 保留。native 全部下载失败在主 LLM 前终止；带图请求主 LLM 失败安全包装为 `mode=native` 摘要（`from None`，不保留原异常 cause/正文，不切 delegated）；部分下载失败成功任务同样附带摘要；无图片的普通 LLM 错误原样传播。成功路径 `process_message` 经共享 `GroupTaskFailureNotifier` 最多提交一次 SUPERUSER 图片汇总卡（`group_text=None` 无群消息，`image_failure_reason_code` 稳定去重键，`error_notify_enabled` 关闭只静默私聊），失败路径 `report_reply_failure` 对图片失败只发一张汇总卡（`reaction_sent` → 群内固定错误文本），debug 干跑绝不通知；带图请求失败的 Agent Run LLM trace 只记录归一化异常类型（不落 `str(exc)`）；图片卡只渲染任务/群/trace/模式/失败阶段/失败数量/错误类型，无 URL/base64/正文/视觉描述
