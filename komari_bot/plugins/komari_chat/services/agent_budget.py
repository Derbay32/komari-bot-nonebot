"""回复 Agent 任务级冻结执行预算（TSK-192）。

预算三元组（最大轮次 / 单轮工具上限 / 整任务工具上限）在任务起点从配置
读取一次并冻结为不可变值对象；任务执行期间配置变更不影响进行中的任务，
只作用于下一个任务。普通 / debug / 简单三条入口共用同一份冻结预算。
