...
class ForgettingService:
    ...
    async def _daily_decay(self) -> None:
        ...
        async with self.pg_pool.acquire() as conn:
            ...
            logger.debug("[KomariMemory] 已衰减所有记忆的重要性")

    async def _delete_low_value_memories(self) -> int:
        """删除重要性=0的低价值记忆（初始评分≤3）。
        ...
        async with self.pg_pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM komari_memory_conversations WHERE importance_current = 0 AND importance_initial <= 3"
            )
            deleted = result.split()[-1] if result else "0"
            logger.debug(f"[KomariMemory] 删除低价值记忆: {deleted} 条")
            return int(deleted)

    async def _fuzzify_and_cleanup_high_value_memories(self) -> int:
        """处理重要性=0的高价值记忆（初始评分>3）。
        ...
        async with self.pg_pool.acquire() as conn:
            ...
            fuzzy_result = await conn.execute(
                "DELETE FROM komari_memory_conversations WHERE importance_current = 0 AND importance_initial > 3 AND is_fuzzy = TRUE"
            )
            deleted_fuzzy = int(fuzzy_result.split()[-1]) if fuzzy_result else 0

            # 2. 模糊化未处理的高价值记忆
            rows = await conn.fetch(
                "SELECT id, summary FROM komari_memory_conversations WHERE importance_current = 0 AND importance_initial > 3 AND is_fuzzy = FALSE"
            )
