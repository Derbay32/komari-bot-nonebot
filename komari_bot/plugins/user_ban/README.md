...
运行时以 5 秒为间隔检查 `komari_user_ban_cache_state` 的单行修订号。修订号未变化时只传输一个整数；发生封禁、解封或到期删除后，写事务会原子推进修订号，各 worker 随后使用 `REPEATABLE READ` 重建一致快照。首次数据库初始化和首次快照加载均使用单飞锁，避免并发请求重复创建连接池或重复拉取全表。
