...
Installation requires explicit ``runtime_check`` and ``send_gate`` callables on
purpose: this plugin never fabricates an always-true send authority.
"""
...
from .handler import RouletteQQHandler
...
def install_roulette_qq_runtime(
    *,
    ...
    ``send_gate`` runs before the atomic claim, ``runtime_check`` runs after it
    and immediately before the platform send.  Both are required keyword
    arguments so no caller can wire an always-true gate by omission.
    """
    ...
    runtime = RouletteQQRuntime(
        handler=RouletteQQHandler(service, delivery, send_gate=send_gate)
    )
