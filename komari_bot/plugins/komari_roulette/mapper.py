...
def transition_from_action_result(
    ...
    *,
    ...
    eliminations = tuple(
        ...
        if old.alive
        and seq in after_by_seq
        and not after_by_seq[seq].alive
    )
...
def _copy_state(state: GameState) -> GameState:
    ...
    players = tuple(
        PlayerSeat(
            ...
            alive=seat.alive,
            inventory={ItemType(item): int(value) for item, value in seat.inventory.items()},
        )
    ...
    copied = GameState(
        ...
        chamber_revision=int(state.chamber_revision),
        turn_seq=int(state.turn_seq),
        ...
        pending_burst=bool(state.pending_burst),
        pending_locks=tuple(int(item) for item in state.pending_locks),
        item_weights={ItemType(item): int(value) for item, value in state.item_weights.items()},
        next_join_seq=int(state.next_join_seq),
    )
...
def _validate_state(state: GameState) -> None:
    ...
    if state.state_revision < 0 or state.chamber_revision < 0 or state.turn_seq < 0:
    ...
    if any(seq <= 0 for seq in sequences) or state.next_join_seq <= max(sequences, default=0):
        raise ValueError("join sequence invariant is invalid")  # noqa: TRY003
    if len(state.players) > MAX_PLAYERS:
        raise ValueError("too many players")  # noqa: TRY003
    for seat in state.players:
        if seat.player.app_id != state.group.app_id or seat.player.group_openid != state.group.group_openid:
            raise ValueError("player group does not match game group")  # noqa: TRY003
        _validate_inventory(seat.inventory)
    if set(state.item_weights) != set(DEFAULT_ITEM_WEIGHTS):
    ...
    if len(set(state.pending_locks)) != len(state.pending_locks) or any(
        not isinstance(item, int) or item <= 0 for item in state.pending_locks
    ):
        raise ValueError("pending locks are invalid")  # noqa: TRY003
    seats_by_seq = {seat.join_seq: seat for seat in state.players}
    if any(target not in seats_by_seq or not seats_by_seq[target].alive for target in state.pending_locks):
    ...
    match state.lifecycle:
        case "waiting":
            ...
            if state.ordered_chamber or state.pending_rewards or state.pending_locks or state.pending_burst:
        ...
        case "completed":
            if state.current_player_seq is not None or sum(seat.alive for seat in state.players) != 1:
                raise ValueError("completed state must have one winner")  # noqa: TRY003
        case "cancelled" | "expired" | "failed":
            if state.current_player_seq is not None:
